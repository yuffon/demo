...
public class Parser {
        ...
        throws IOException, JavaScriptException
    {
        ...
        if (tt != ts.EOF && tt != ts.EOL
            && tt != ts.SEMI && tt != ts.RC)
            {
                int version = 0;//Context.getContext().getLanguageVersion();
    ...
    public Object primaryExpr(TokenStream ts, Source source)
        throws IOException, JavaScriptException
    {
        int tt;
                ...
                if (!matched) {
                    ...
                    do {
                        ...
                        if (tt == ts.COMMA) {
                            ...
                            nf.addChildToBack(pn, assignExpr(ts, source, false)); // assignExpr can generate constraints!!
                        }
                    } while (ts.matchToken(ts.COMMA));
                    mustMatchToken(ts, ts.RB, "msg.no.bracket.arg");
                }
                source.append((char)ts.RB);
                return nf.createArrayLiteral(pn); // sensitive events enumeration in this form!!
            }
        ...
        case TokenStream.LC: {
            ...
            source.append((char)ts.RC);
            return nf.createObjectLiteral(pn); // can creat sensitive events!!
        }
